...
在程序设计中，有相当一类求一组解、或求全部解或求最优解的问题，例如八皇后问题等，不是根据某种确定的计算法则，而是利用试探和**回溯(Backtracking)**的搜索技术求解。回溯法也是设计递归过程的一种重要方法，它的求解过程实质上是一个先序遍历一棵“状态树”的过程，只是这棵树不是遍历先前预先建立的，而是隐含在遍历过程中。

> TODO 四皇后问题与幂集问题

### 树的计数

> 二叉树的相似是指：二者都为空树或者二者均不为空树，且它们的左右子树分别相似。
> 二叉树的等价是指：二者不仅相似，而且所有对应结点上的数据元素均相同。

二叉树的计数问题是讨论具有n个结点、互不相似的二叉树的数目bn。

> TODO 根据递推式推导二叉树的数量 太特么长了 看不懂 先放放。
